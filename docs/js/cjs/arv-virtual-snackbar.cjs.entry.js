...
const VirtualSnackbar = class {
    constructor(hostRef) {
        ...
        this.arvVirtualSnackbarClose = index.createEvent(this, "arvVirtualSnackbarClose", 7);
    }
    async durationCheck() {
        this.init();
    }
    ...
    render() {
        const cls = Object.assign({ top: this.yPosition === 'top', bottom: this.yPosition === 'bottom', left: this.xPosition === 'left', right: this.xPosition === 'right', verticalCenter: this.yPosition === 'center', horizontalCenter: this.xPosition === 'center', hideClose: this.hideClose }, helpers.generateAttrValue(this.color));
        return (index.h(index.Host, { class: cls }, index.h("div", { class: "snackbar" }, this.icon && (index.h("arv-icon", { class: "sb-icon", icon: this.icon })), index.h("arv-text", { wrap: "nowrap" }, this.message), index.h("slot", null), !this.hideClose && (index.h("arv-button", { class: "sb-btn", onClick: this.closeBtnClick, color: this.color, icon: "clear", "is-icon": true })))));
    }
...
exports.arv_virtual_snackbar = VirtualSnackbar;

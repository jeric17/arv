...
const Tabs = class {
    constructor(hostRef) {
        ...
        this.arvTabClick = index.createEvent(this, "arvTabClick", 7);
    }
    alignmentChanged() {
        setTimeout(() => {
            this.counter++;
    ...
    render() {
        const hostCls = Object.assign(Object.assign(Object.assign({}, helpers.generateAttrValue(this.color)), helpers.generateAttrValue(this.tabAlignment)), { noActiveHeader: !Boolean(this.activeHeader) });
        const tabNames = JSON.parse(this.tabNames);
        return (index.h(index.Host, { class: hostCls }, index.h("div", { class: "header" }, tabNames.map((item, index$1) => {
            const tabCls = {
                tabName: true,
                active: index$1 === this.activeIndex,
            };
            return (index.h("div", { onClick: () => this.tabClick(index$1), class: tabCls }, item.icon && (index.h("arv-icon", { icon: item.icon })), index.h("span", null, item.name), index.h("div", { class: "tempBorder" })));
        }), index.h("span", { class: "filler" }), index.h("div", { class: "border", style: this.borderCls(this.activeIndex) })), index.h("slot", null)));
    }

...
const MenuItem = class {
    ...
    render() {
        return (h(Host, { onClick: () => this.arvMenuSelect.emit(this.value) }, !this.hideValue && (h("arv-text", { wrap: "nowrap" }, h("slot", { name: "value" }))), h("slot", null)));
    }
};
...
const Select = class {
    ...
    render() {
        ...
        return (h(Host, { class: hostCls, style: styles }, h("label", { style: labelStyle }, this.label), h("div", { class: "control" }, h("div", { onClick: () => this.open(), class: "select" }, h("span", { class: "value" }, this.value || this.placeholder), h("slot", { name: "value" }), h("arv-icon", { icon: "keyboard_arrow_down" })), h("div", { onBlur: () => this.blur(), tabIndex: -1, class: "items" }, h("slot", null)))));
    }
    close() {
        return delay(() => {
            this.isOpen = false;
            ...
            const control = (() => {
                if (item.type === 'color') {
                    return (h("arv-select", { label: item.name || 'color', labelWidth: labelWidth, "flex-direction": "row", value: item.value || 'Select', selectChange: data => {
                        ...
                        }, full: true }, h("arv-menu-item", { value: "" }, "None"), this.colors.map((color) => (h("arv-menu-item", { value: color }, color)))));
                }
                ...
                if (item.type === 'oneOf') {
                    return (h("arv-select", { label: item.name, labelWidth: labelWidth, "flex-direction": "row", value: item.value || 'Select', selectChange: data => {
                            this.dataChange(data, index);
                        }, full: true }, h("arv-menu-item", { value: "" }, "None"), item.data.map((d) => (h("arv-menu-item", { value: d }, d)))));
                }
                if (item.type === 'string') {
...
const DocNav = class {
    ...
    render() {
        return (h("arv-side-pane", null, h("arv-list", null, this.items.map(item => (h("arv-list-item", { "active-color": "light", isActive: this.selectedItem === item, onClick: () => this.click(item) }, item))))));
    }
};

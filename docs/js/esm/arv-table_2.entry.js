...
const Table = class {
    ...
    render() {
        ...
        return (h(Host, { class: cls }, h("div", { class: "heading" }, Boolean(this.tableTitle) && (h("h1", { class: "table-title" }, this.tableTitle)), h("slot", { name: "header" })), h("table", Object.assign({ class: "table" }, this.tableProps), h("thead", null, h("tr", { class: "tr" }, this.tableHeadersData.map(headerItem => (h("th", { onClick: this.thItemClick.bind(this, headerItem), class: "th" }, headerItem))), Boolean(this.controls.length) && (h("th", { class: "th" })))), h("tbody", null, this.tableDataArray.map(rowData => {
            const [id, ...dataBody] = rowData;
            return (h("tr", { "data-id": id, class: trClass, onClick: this.trItemClick.bind(this, rowData) }, dataBody.map(rowItem => (h("td", { class: "td", onClick: this.tdItemClick.bind(this, rowItem) }, this.generateRowItem(rowItem)))), Boolean(this.controls.length) && (h("td", { class: "td controls" }, h("arv-flex", { justify: "flex-end" }, this.controls.map(ctrlItem => (h("arv-button", { color: this.color, variant: "ghost", icon: ctrlItem.icon, onClick: () => ctrlItem.fn(rowData), "is-icon": true }))))))));
...
const MyThemeSection = class {
    ...
    render() {
        ...
        return (h("arv-flex", { fullHeight: true, layout: "column", padded: true }, h("arv-text", null, "Theming"), h("arv-divider", null), h("arv-text", null, "Arv ui library uses css variables to control the overall theme"), h("arv-divider", null), h("arv-text", null, "Current theme settings:"), keys.map((item) => (h("arv-flex", { items: "end" }, h("arv-input", { name: item, label: item, value: this.themeSettings[item] }), h("arv-divider", { "is-vertical": true }), h("arv-input", { type: "color", name: item, value: this.themeSettings[item] })))), h("arv-input", { label: "--radius", value: this.radius }), h("arv-divider", null)));
    }
};
